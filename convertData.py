...
__city_cubs = ['海门', '鄂尔多斯', '招远', '舟山', '齐齐哈尔', '盐城', '赤峰', '青岛', '乳山', '金昌', '泉州', '莱西', '日照',
...
__shengfens = ['重庆市', '河北省', '山西省', '辽宁省', '吉林省', '黑龙江省', '江苏省', '浙江省', '安徽省', '福建省', '江西省',
             '山东省', '河南省', '湖北省', '湖南省', '广东省', '海南省', '四川省', '贵州省', '云南省', '陕西省', '甘肃省', '青海省']
def convert(data):
    data = list(_flatten(list(map(list, data))))
    for i in range(0, len(data)):
        for city_cub in __city_cubs:
            pattren = re.compile(str(city_cub) + '.*[\u4e00-\u9fa5]*', re.S)
            data[i] = re.sub(pattren, city_cub, data[i])
